...
#proceedings_file = open('proceedings_publisher_file.csv', 'w')
with open('proceedings_publisher_file.csv', 'w',encoding='utf-8') as proceedings_file:
  ...
  for proceedings in root.iter('proceedings'):
    ...
    for node in proceedings.getiterator():
      if node.tag=='publisher':
        proceedings_file_line.append(node.text)
        href = node.get('href')
        proceedings_file_line.append(href)
