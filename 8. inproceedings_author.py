...
with open('inproceedings_author_file.csv', 'w',encoding='utf-8') as inproceedings_file:
  ...
  for inproceedings in root.iter('inproceedings'):
    ...
    for node in inproceedings.getiterator():
      if node.tag=='author':
        ...
        inproceedings_file_line.append(bibtex)
        orcid = node.get('orcid')
        inproceedings_file_line.append(orcid)
